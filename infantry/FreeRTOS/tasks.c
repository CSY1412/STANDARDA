 ...
 *如果xTask引用的任务当前处于挂起状态，则返回pdtrue；如果xTask*引用的任务处于任何其他状态，则返回pdfalse。
 */
 ...
 * automatically upon the creation of the first task.
 实用工具来准备调度程序使用的所有列表。这在创建第一个任务时被自动调用。
 */
 ...
 * void prvIdleTask( void *pvParameters );
 ...
 空闲任务，它作为所有任务被实现为一个永无止境的循环。
 *空闲任务在*创建第一个用户任务时自动创建并添加到就绪列表中
。*portTask_function_proto()宏用于允许端口/编译器特定的*语言扩展。
 此函数的等效原型是：*voidprvidletask(void*pvproperties)；
 *
 */
 ...
 *实用程序释放调度程序分配的所有内存以容纳tcb，
 *包括tcb所指向的堆栈。*这不会释放由任务本身分配的内存
 (即，从任务应用程序代码中调用pvportmalloc分配的内存*)。
 */
#if ( INCLUDE_vTaskDelete == 1 )
 ...
 *仅供闲置任务使用。
 这将检查是否有任何内容被放置在等待删除的任务列表中。
 如果是这样，则清除该任务*并删除其tcb。
 */
 ...
 *当前正在执行的任务正在进入阻塞状态。将任务添加到*当前任务列表或溢出延迟任务列表中。**
 */
